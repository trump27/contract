        ...
        <tr class="<?=!empty($contract->client->partner_id)?'active':''?>">
            ...
            <td class="actions">
                ...
                if (!empty($contract->file))
                    echo $this->Html->link('', ['controller' => 'Contracts', 'action' => 'download', $contract->id], ['title' => urldecode($contract->file), 'class' => 'btn btn-default btn-xs glyphicon glyphicon-file alert-warning']);
                ?>
            </td>
            ...
            <td><?= $this->my->trunc(h($contract->remarks),20) ?></td>
